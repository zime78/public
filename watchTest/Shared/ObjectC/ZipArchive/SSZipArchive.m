...
//        NSStringEncoding encoding = CFStringConvertEncodingToNSStringEncoding(kCFStringEncodingDOSLatinUS);
        NSString* strPath = [NSString stringWithCString:filename encoding:CFStringConvertEncodingToNSStringEncoding(kCFStringEncodingEUC_KR)];
...
int _zipOpenEntry(zipFile entry, NSString *name, const zip_fileinfo *zipfi, int level, NSString *password, BOOL aes)
{
    const char *encodedString =  [name cStringUsingEncoding:CFStringConvertEncodingToNSStringEncoding(kCFStringEncodingEUC_KR)];
    return zipOpenNewFileInZip5(entry, encodedString, zipfi, NULL, 0, NULL, 0, NULL, 0, 0, Z_DEFLATED, level, 0, -MAX_WBITS, DEF_MEM_LEVEL, Z_DEFAULT_STRATEGY, password.UTF8String, aes, 0);
